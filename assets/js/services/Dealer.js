...
const formUpdate = document.querySelector('#formulario') || document.createElement('formulario');
...
const contenedor = document.getElementById('content-page') || document.createElement('content-page');
...
function validarFormulario(e) {
    ...
    if([nombre.value,paterno.value,materno.value,phone.value,email.value].includes('')){
    ...
    const mostrar = (dealers) => {
        dealers.data.forEach(dealer => {
            ...
            if( dealer.role_id == 2){
               ...
               divf.className = 'd-flex w-100 flex-wrap align-items-center justify-content-between gap-2'
               ...
               divbg.className = 'bg-light-secondary p-3 rounded me-auto mb-3'
               divbg.innerHTML = ` <h6><i class="fas fa-store me-2"></i>
                                    Dirección de la Tienda:
                                   </h6>
                                    <span class="fs-6 text-secondary">
                                     Av. Lomas de Cortés #456 Colonia Alta Palmira, Temixco Morelos
                                    </span>`
...
function validarFormularioUpdate() {
        if([nombreU.value,paternoU.value,maternoU.value,phoneU.value,emailU.value].includes('')){
            alert('Todos los campos son obligatorios')
            return true;
            }else{
                 actualizarDealer()
