...
const registerForm =
  document.querySelector("#registerForm") || document.createElement("form");
const inputName =
...
inputStores.addEventListener("keyup", (e) => {
  if (e.target.value.length > 0) {
    filterStores = allStores.filter((store) =>
  ...
  fetch(API_URI + "/stores", {
    ...
    .then((data) => {
      if (data.success === true) {
        ...
        NotifyService.notificatonError(
          "Ha ocurrido un error al registrar la tienda, revisa los datos ingresados"
        );
      }
    });
}
    ...
    card.innerHTML += `
              ...
              <div class="card-body">
                  ...
                  <div class="d-flex justify-content-between my-3 mt-4">
                      <button type="submit" id="btnEliminar" class="btn btn-outline-danger" onclick="deleteStore(${store.id})">
